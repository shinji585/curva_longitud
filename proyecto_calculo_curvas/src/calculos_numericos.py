...
def longitud_arco(f, a, b, n=100, h=0.0001):
    ...
    def integrado(x):
        try:
            ...
            # En caso de error en el cálculo, devolver 1 (mínimo posible)
            print(f"Advertencia en punto x={x}: {e}")
            return 1.0
...
def calcular_longitud_con_calibracion(longitud_piexels, factor_escala):
    """
    Convierte una longitud en pixeles a unidades reales usando un factor de escala.
    
    Args:
        longitud_piexels: longitud en pixeles
        factor_escala: factor de conversion (unidades reales / pixel)
       
    Returns:
        longitud en unidades reales
    """
    return longitud_piexels * factor_escala